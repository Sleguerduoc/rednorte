-- ===== BLOQUE 1: pacientes_db =====
-- ids 1-2: pacientes de LOGIN (antes en servicio-pacientes/data.sql, hoy desactivado).
--          Mismo RUT/datos que data.sql para no romper el login CLIENTE
--          (UserService busca el username ingresado contra el RUT de estos registros).
-- ids 13-24: pacientes de DEMOSTRACION (lista de espera / agenda del dia).
...
SELECT setval(pg_get_serial_sequence('pacientes','id'), 24, true);

-- ===== BLOQUE 2: lista_db =====
-- paciente_id apunta a los ids de demostracion (13-24) del bloque anterior.
